...
class CalculatorController {
  ...
  /**
   * Power endpoint handler (bonus feature)
   * @param {Object} req - Express request object
   * @param {Object} res - Express response object
   */
  power(req, res) {
    try {
      const { num1, num2 } = this.#extractNumbers(req);
      const result = this.calculator.power(num1, num2);

      res.status(200).json(
        this.#formatSuccessResponse('power', num1, num2, result)
      );
    } catch (error) {
  ...
  /**
   * Square root endpoint handler (bonus feature)
   * @param {Object} req - Express request object
   * @param {Object} res - Express response object
   */
  sqrt(req, res) {
    try {
      const num = this.#extractSingleNumber(req);
      const result = this.calculator.sqrt(num);

      res.status(200).json(
        this.#formatSuccessResponse('square_root', num, undefined, result)
      );
    } catch (error) {
