...
/**
 * Middleware to validate a single number (num)
 * Used for operations like square root
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 * @param {Function} next - Express next function
 */
const validateSingleNumber = (req, res, next) => {
  try {
    // Extract number from query or body
    ...
    if (num === undefined) {
      return res.status(400).json({
        ...
        examples: {
          query: '/calculator/sqrt?num=16',
          body: 'POST with body: {"num": 16}'
        },
        timestamp: new Date().toISOString()
      });
    }

    // Validate that input can be converted to number
    ...
    if (parsedNum > MAX_SAFE_NUMBER || parsedNum < MIN_SAFE_NUMBER) {
      return res.status(400).json({
        success: false,
        error: 'num is outside safe number range',
        provided: parsedNum,
        range: { min: MIN_SAFE_NUMBER, max: MAX_SAFE_NUMBER },
        timestamp: new Date().toISOString()
      });
    ...
    // Store validated number in request for controller use
    req.validatedNumber = {
      num: parsedNum
    };
...
const customValidations = {
  ...
  /**
   * Validation specific to square root operation
   * Adds check for negative numbers
   */
  squareRoot: (req, res, next) => {
    const { num: queryNum } = req.query;
    const { num: bodyNum } = req.body || {};
    const num = parseFloat(queryNum || bodyNum);
  ...
  /**
   * Validation for power operation
   * Adds checks for reasonable exponent values
   */
  power: (req, res, next) => {
    const { num2: queryNum2 } = req.query;
    const { num2: bodyNum2 } = req.body || {};
    const exponent = parseFloat(queryNum2 || bodyNum2);
