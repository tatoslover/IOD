 ...
 *
 * Basic test suite for the enhanced calculator MVC application.
...
describe('Calculator API Tests', () => {
  describe('GET /calculator endpoints', () => {
    ...
    test('GET /calculator/power should return correct result', async () => {
      const response = await request(app)
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.result).toBe(8);
      expect(response.body.operation).toBe('power');
    });

    test('GET /calculator/sqrt should return correct result', async () => {
      const response = await request(app)
  ...
  describe('Error Handling', () => {
    ...
    test('should return 400 for negative square root', async () => {
      const response = await request(app)
        .get('/calculator/sqrt?num=-4')
        .expect(400);

      expect(response.body.success).toBe(false);
      expect(response.body.error).toContain('Cannot calculate square root of negative number');
    });
  });
  ...
  describe('Root Endpoints', () => {
    test('GET / should serve calculator HTML', async () => {
      ...
      expect(response.text).toContain('Enhanced Calculator');
    });
    ...
    test('GET /api should return API documentation', async () => {
      ...
      expect(response.body.service).toBe('Enhanced Calculator API');
